...
public class J9BCUtil {
	...
	private static void dumpStackMapTable(PrintStream out, J9ROMClassPointer romclass, J9ROMMethodPointer romMethod, long flags) throws CorruptDataException {
		...
		if (stackMapMethod.notNull()) {
			...
			while (i < stackMapCount.intValue()) {
				mapPC++; // TODO not sure if this is right
	...
	static U8Pointer dumpUnsetFields(PrintStream out, J9ROMClassPointer classfile, U8Pointer slotData) throws CorruptDataException
	{
		...
		slotData = slotData.add(2);
		out.println("        unset_fields:");
		J9ROMConstantPoolItemPointer constantPool = ConstantPoolHelpers.J9_ROM_CP_FROM_ROM_CLASS(classfile);
		for (int i = 0; i < unsetFieldCount; i++) {
			int nameAndSignatureIndex = new U16(slotData.at(0)).leftShift(8).add(slotData.at(1)).intValue();
			J9ROMNameAndSignaturePointer nameAndSignature = J9ROMNameAndSignaturePointer.cast(constantPool.add(nameAndSignatureIndex));
			J9UTF8Pointer name = nameAndSignature.name();
			J9UTF8Pointer signature = nameAndSignature.signature(); // TODO not sure if signature was marked
			out.println("          " + J9UTF8Helper.stringValue(name) + ":" + J9UTF8Helper.stringValue(signature));
			slotData = slotData.add(2);
		}
		return slotData;
	}
