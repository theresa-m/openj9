...
newArrayHelper(JNIEnv *env, jclass componentType, jint length, bool isNullRestricted, jobject initialValueJni)
{
	...
	if (isNullRestricted) {
		...
		/* The initial value class must be the same as the array type since value classes can't be extended
		 * and the superclass java/lang/Object can't be assigned.*/
	...
	if (isNullRestricted) {
		for (int index = 0; index < length; index++) {
			/* It's not necessary to copy the inital value since a value must
			 * have strict final instance fields.
			 */
