...
#if defined(J9VM_OPT_VALHALLA_FLATTENABLE_VALUE_TYPES)
			+ _totalFlatFieldShortBytes + _totalFlatFieldByteBytes;
#endif /* defined(J9VM_OPT_VALHALLA_FLATTENABLE_VALUE_TYPES) */	
		;
...
ObjectFieldInfo::calculateFlatFieldSize()
{
	U_8 totalSize = (U_8)-1;
	if (isValue()) {
		U_32 accumulator = _superclassFlatFieldsSize;
		accumulator += (_totalObjectCount * _referenceSize) + (_totalSingleCount * sizeof(U_32)) + (_totalDoubleCount * sizeof(U_64))
			+ (_totalShortCount * sizeof(U_16)) + (_totalByteCount * sizeof(U_8));
		accumulator += _totalFlatFieldDoubleBytes + _totalFlatFieldRefBytes + _totalFlatFieldSingleBytes
			+ _totalFlatFieldShortBytes + _totalFlatFieldByteBytes;
		
		/* Only record the flat value size if there's a possibility of this object being small enough to flatten.*/
		if (accumulator <= sizeof(U_64)) {
