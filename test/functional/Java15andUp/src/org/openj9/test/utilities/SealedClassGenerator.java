 ...
 *******************************************************************************/
 ...
 public class RecordClassGenerator implements Opcodes {
    ...
    public static byte[] generateSubclassIllegallyExtendingSealedSuperclass(String className, String superclassName) {
        ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES);
        cw.visit(V15 | V_PREVIEW, ACC_SUPER, className, null, superclassName, null);
        cw.visitEnd();
        return cw.toByteArray();
    }

    public static byte[] generateSubinterfaceIllegallyExtendingSealedSuperinterface(String className, String superinterfaceName) {
        ClassWriter cw = new ClassWriter(ClassWriter.COMPUTE_FRAMES);
        cw.visit(V15 | V_PREVIEW, ACC_INTERFACE | ACC_ABSTRACT, className, null, superinterfaceName, null);
        cw.visitEnd();
        return cw.toByteArray();
    }
 }
