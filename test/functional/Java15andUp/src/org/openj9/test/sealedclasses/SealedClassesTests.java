...
public class SealedClassesTests {
    ...
    public void test_loadIllegalSubclassOfSealedClass() {
        CustomClassLoader classloader = new CustomClassLoader();
        byte[] bytes = SealedClassGenerator.generateSubclassIllegallyExtendingSealedSuperclass(name, TestClassSealed.class.getName());
        Class<?> clazz = classloader.getClass(name, bytes);
    }
    ...
    public void test_loadIllegalSubinterfaceOfSealedInterface() {
        CustomClassLoader classloader = new CustomClassLoader();
        byte[] bytes = SealedClassGenerator.generateSubinterfaceIllegallyExtendingSealedSuperinterface(name, TestInterfaceSealed.class.getName());
        Class<?> clazz = classloader.getClass(name, bytes);
    }
}
